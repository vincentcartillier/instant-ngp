...
std::vector<float> Testbed::make_gaussian_kernel(const uint32_t kernel_size, const float sigma) {
...
void Testbed::track_pose_gaussian_blur_nerf_slam(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	track_pose_gaussian_blur_nerf_slam_step(target_batch_size, m_nerf.training.counters_rgb_track, stream);
...
void Testbed::track_pose_gaussian_blur_nerf_slam_step(uint32_t target_batch_size, Testbed::NerfCounters& counters, cudaStream_t stream) {
    ...
    uint32_t* existing_ray_mapping_gpu = std::get<21>(scratch);
	float* super_ray_gradients = std::get<22>(scratch);
    ...
    std::vector<float> kernel = make_gaussian_kernel(kernel_window_size, sigma);
    ...
    //NOTE: create a mapping to avoid computing gradient multiple times for the same ray
    std::vector<uint32_t> existing_ray_mapping(counters.rays_per_batch);
    std::unordered_map<uint32_t, uint32_t> tmp_dict;
	Eigen::Vector2i resolution = m_nerf.training.dataset.metadata[m_nerf.training.indice_image_for_tracking_pose].resolution;
    for (size_t k=0; k<counters.rays_per_batch; ++k) {
        uint32_t x = m_nerf.training.sampled_pixels_for_tracking[2*k];
        uint32_t y = m_nerf.training.sampled_pixels_for_tracking[2*k+1];
        uint32_t key = x + y * resolution.x();
        if (tmp_dict.count(key) == 0) {
            existing_ray_mapping[k] = k;
            tmp_dict[key] = k;
        } else {
            existing_ray_mapping[k] = tmp_dict[key];
        }
    }
    CUDA_CHECK_THROW(
       cudaMemcpy(
