...
void Testbed::set_nerf_network_render_blurry(bool render_blurry) {
    m_nerf_network->set_render_blurry(render_blurry);
}
...
void Testbed::reset_network(bool clear_density_grid) {
	...
	*/
	...
	if (to_lower(encoding_config.value("otype", "OneBlob")).find("grid") != std::string::npos) {
        ...
        //NOTE: modifying this to allow loading same number of parameters
        if (encoding_config.contains("per_level_scale")) {
            m_per_level_scale = encoding_config["per_level_scale"];
        } else {
		    m_per_level_scale = encoding_config.value("per_level_scale", 0.0f);
        }
...
void Testbed::render_frame_main(
    ...
    if ((m_manually_set_focal_length.x() > 0.0) && (m_manually_set_focal_length.y() > 0.0)){
	    focal_length[0] = m_manually_set_focal_length.x();
	    focal_length[1] = m_manually_set_focal_length.y();
    } else {
	    focal_length = calc_focal_length(device.render_buffer_view().resolution, relative_focal_length, m_fov_axis, m_zoom);
    }

    vec2 screen_center;
    if ((m_manually_set_screen_center.x() > 0.0) && (m_manually_set_screen_center.y() > 0.0)){
	    screen_center[0] = m_manually_set_screen_center.x();
	    screen_center[1] = m_manually_set_screen_center.y();
    } else {
	    screen_center = render_screen_center(orig_screen_center);
    }
...
void Testbed::save_snapshot(const fs::path& path, bool include_optimizer_state, bool compress) {
    ...
    //NOTE: save that info so we can reload the network afterwards.
    m_network_config["encoding"]["per_level_scale"] = m_per_level_scale;
